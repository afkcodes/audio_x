...
const BASE_EVENT_CALLBACK_MAP: EventListenerCallbackMap = {
  ...
  TIME_UPDATE: (e: Event, audioInstance: HTMLAudioElement) => {
    ...
    notify(
      'AUDIO_STATE',
      {
        playbackState: audioInstance.paused ? audioState?.playbackState : PLAYBACK_STATE.PLAYING,
        progress: audioInstance?.currentTime,
        error: { code: null, message: '', readable: '' },
        bufferedDuration,
      },
  ...
  VOLUME_CHANGE: (e: Event) => {
    console.log('STATUS', e.type);
    notify('AUDIO_STATE', {}, 'audiox_baseEvents_state');
  },
  ...
  SEEKED: (e: any, audioInstance: HTMLAudioElement) => {
    ...
    notify(
      'AUDIO_STATE',
      {
        playbackState: audioState.playbackState === 'paused' ? 'paused' : audioState.playbackState,
        progress: audioInstance?.currentTime,
        error: { code: null, message: '', readable: '' },
        bufferedDuration,
      } as AudioState,
